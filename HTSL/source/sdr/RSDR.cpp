...
void RSDR::activate(int subIter, float leak) {
	// Activate
...
void RSDR::inhibit(int subIter, float leak, const std::vector<float> &activations, std::vector<float> &states) {
	states.clear();
...
void RSDR::learn(float learnFeedForward, float learnRecurrent, float learnLateral, float learnThreshold, float sparsity) {
	...
	for (int hi = 0; hi < _hidden.size(); hi++) {
		...
		if (learn > 0.0f) {
			for (int ci = 0; ci < _hidden[hi]._feedForwardConnections.size(); ci++)
				_hidden[hi]._feedForwardConnections[ci]._weight += learnFeedForward * visibleErrors[_hidden[hi]._feedForwardConnections[ci]._index];

			for (int ci = 0; ci < _hidden[hi]._recurrentConnections.size(); ci++)
				_hidden[hi]._recurrentConnections[ci]._weight += learnRecurrent * hiddenErrors[_hidden[hi]._recurrentConnections[ci]._index];
		}
...
void RSDR::learn(const std::vector<float> &attentions, float learnFeedForward, float learnRecurrent, float learnLateral, float learnThreshold, float sparsity) {
	...
	for (int hi = 0; hi < _hidden.size(); hi++) {
		...
		if (learn > 0.0f) {
			for (int ci = 0; ci < _hidden[hi]._feedForwardConnections.size(); ci++)
				_hidden[hi]._feedForwardConnections[ci]._weight += learnFeedForward * attentions[hi] * visibleErrors[_hidden[hi]._feedForwardConnections[ci]._index];

			for (int ci = 0; ci < _hidden[hi]._recurrentConnections.size(); ci++)
				_hidden[hi]._recurrentConnections[ci]._weight += learnRecurrent * attentions[hi] * hiddenErrors[_hidden[hi]._recurrentConnections[ci]._index];
		}

		for (int ci = 0; ci < _hidden[hi]._lateralConnections.size(); ci++)
			_hidden[hi]._lateralConnections[ci]._weight = std::max(0.0f, _hidden[hi]._lateralConnections[ci]._weight + learnLateral * attentions[hi] * (_hidden[hi]._state * _hidden[_hidden[hi]._lateralConnections[ci]._index]._state - sparsitySquared));
