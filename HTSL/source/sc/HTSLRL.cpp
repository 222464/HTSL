...
void HTSLRL::update(float reward, std::mt19937 &generator) {
	...
	float alphaTdError = _qAlpha * tdError;

	for (int ni = 0; ni < _qNodes.size(); ni++) {
		for (int ci = 0; ci < _qNodes[ni]._firstHiddenConnections.size(); ci++) {
			_qNodes[ni]._firstHiddenConnections[ci]._weight += alphaTdError * _qNodes[ni]._firstHiddenConnections[ci]._trace;
			
			if (ci % 2 == 0)
				_qNodes[ni]._firstHiddenConnections[ci]._trace = std::max((1.0f - _qTraceDecay) * _qNodes[ni]._firstHiddenConnections[ci]._trace, _htsl.getLayers().front()._rsc.getHiddenStatePrev(_qNodes[ni]._firstHiddenConnections[ci]._index));
			else
				_qNodes[ni]._firstHiddenConnections[ci]._trace = std::max((1.0f - _qTraceDecay) * _qNodes[ni]._firstHiddenConnections[ci]._trace, _htsl.getLayers()[1]._predictionNodes[_qNodes[ni]._firstHiddenConnections[ci]._index]._statePrev);
		}
	}

	float actionLearn = tdError > 0.0f ? _actionAlpha : 0.0f;
	...
	for (int ni = 0; ni < _actionNodes.size(); ni++) {
		...
		for (int ci = 0; ci < _actionNodes[ni]._firstHiddenConnections.size(); ci++) {
			if (ci % 2 == 0)
				sum += _actionNodes[ni]._firstHiddenConnections[ci]._weight * _htsl.getLayers().front()._rsc.getHiddenStatePrev(_qNodes[ni]._firstHiddenConnections[ci]._index);
			else
				sum += _actionNodes[ni]._firstHiddenConnections[ci]._weight * _htsl.getLayers()[1]._predictionNodes[_qNodes[ni]._firstHiddenConnections[ci]._index]._statePrev;
		}
