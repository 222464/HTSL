...
namespace hyp {
	...
	public:
		float _xi;
		int _annealingIterations;
		int _annealingSamples;
		float _annealingPertStdDev;
		float _annealingDecay;
		...
		void setCurrentVariables(const std::vector<float> &vars) {
			assert(vars.size() == _currentVariables.size());

			_currentVariables = vars;
		}
