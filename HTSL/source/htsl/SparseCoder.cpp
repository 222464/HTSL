...
void SparseCoder::activate(float activationLeak) {
	// Activate
	for (int hi = 0; hi < _hidden.size(); hi++) {
		...
		_hidden[hi]._activation = std::max(activationLeak, sum);
	}
...
void SparseCoder::learn(float alpha, float beta, float gamma, float sparsity) {
	...
	for (int hi = 0; hi < _hidden.size(); hi++) {
		if (_hidden[hi]._state != 0.0f) {
			for (int vi = 0; vi < _visible.size(); vi++)
				_hidden[hi]._visibleHiddenConnections[vi]._weight += alpha * _hidden[hi]._state * visibleErrors[vi];
		}

		for (int hio = 0; hio < _hidden.size(); hio++)
			_hidden[hi]._hiddenHiddenConnections[hio]._weight = std::max(0.0f, _hidden[hi]._hiddenHiddenConnections[hio]._weight + beta * (_hidden[hi]._state * (_hidden[hio]._state > 0.0f ? 1.0f : 0.0f) - sparsitySquared));
